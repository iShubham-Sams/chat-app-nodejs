...
class ApiError extends Error {
  statusCode: number;
  errorName: string;
  constructor(message = "Something went wrong", statusCode: number, errorName: string) {
    super(message);
  ...
  static ServerError(message: string) {
    return new ApiError(message, HttpStatusCode.INTERNAL_SERVER_ERROR, "ServerError");
  }
